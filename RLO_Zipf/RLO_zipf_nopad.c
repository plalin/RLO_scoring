...
double pr_rlen_s_given_rlen_x(int s, int x) {
    return (s == x) ? 1.0 : 0.0;  // s와 x가 같으면 1.0, 다르면 0.0 반환
}
...
double pr_key_w_given_rlen_s(int keyword_num_documents, int s, double pr_rlen_prime_s_val, double zipf_prob) {
    double pr_kw_w = zipf_prob;  // 현재 키워드에 대응되는 zipf_prob 값
    double pr_rlen_s_given_kw_w = pr_rlen_s_given_rlen_x(s, keyword_num_documents);  // Pr(rlen'=s|kw=w)를 계산
...
#pragma omp parallel for
    for (int s = s_min; s <= s_max; s++) {
        ...
        for (int x = s_min; x <= s_max; x++) {
            pr_rlen_prime_s[s - s_min] += pr_rlen_x(keyword_num_documents, zipf_probs, num_keywords, x) * pr_rlen_s_given_rlen_x(s, x);
        }
...
double H_s(int* keyword_num_documents, double* zipf_probs, int num_keywords, int s, double* pr_rlen_prime_s_arr, int s_min, int s_max) {
    ...
    for (int i = 0; i < num_keywords; i++) {
        double pr_key_w_rlen_s = pr_key_w_given_rlen_s(keyword_num_documents[i], s, pr_rlen_prime_s_val, zipf_probs[i]);
...
int main() {
    ...
    // 결과 출력
    printf("RLO_zipf_nopad: %lf\n", rlo_value);

    // 결과 파일에 저장
    FILE* output_file = fopen("RLO_zipf_nopad_1000.txt", "w");
    if (output_file != NULL) {
        fprintf(output_file, "RLO_zipf_nopad_1000: %lf\n", rlo_value);
        fclose(output_file);
        printf("RLO_zipf_nopad value saved to RLO_zipf_nopad_1000.txt\n");
    }
    else {
