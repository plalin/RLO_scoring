...
int main() {
    ...
    // 결과 출력
    printf("RLO_zipf_max: %lf\n", rlo_value);

    // 결과 파일에 저장
    FILE* output_file = fopen("RLO_zipf_max_1000.txt", "w");
    if (output_file != NULL) {
        fprintf(output_file, "RLO_zipf_max_1000: %lf\n", rlo_value);
        fclose(output_file);
        printf("RLO_zipf_max value saved to RLO_zipf_max_1000.txt\n");
    }
    else {
