...
void calculate_and_save_rlo(const char* input_file, const char* output_file, int W) {
    ...
    if (read_csv(input_file, keyword_num_documents, W) != 0) {
        ...
        exit(EXIT_FAILURE);  // 파일을 읽지 못하면 종료
    }
...
int main() {
    // RLO 계산 및 파일 저장
    calculate_and_save_rlo("C:/dataset/picked_pairs(500).csv", "RLO_uniform_rp_500.txt", 500);
    calculate_and_save_rlo("C:/dataset/picked_pairs(1000).csv", "RLO_uniform_rp_1000.txt", 1000);
    calculate_and_save_rlo("C:/dataset/picked_pairs(2000).csv", "RLO_uniform_rp_2000.txt", 2000);
