...
int main() {
    ...
    // CSV 파일 읽기
    const char* filename = "C:/dataset/picked_pairs(1000).csv";
    ...
    int s_max = 100962;    // 최대 s 값

    // RLO 값 계산
    double rlo_value = RLO_uniform_nopad(keyword_num_documents, num_keywords, s_min, s_max);

    // 결과 출력
    printf("RLO_uniform_RP: %lf\n", rlo_value);

    // 결과 파일에 저장
    FILE* output_file = fopen("RLO_uniform_RP_1000.txt", "w");
    if (output_file != NULL) {
        fprintf(output_file, "RLO_uniform_RP_1000: %lf\n", rlo_value);
        fclose(output_file);
        printf("RLO_uniform_nopad value saved to RLO_uniform_RP_1000.txt\n");
    }
    else {
