...
int main() {
    ...
    // 결과 파일에 저장
    FILE* output_file = fopen("RLO_uniform_nopad_1000.txt", "w");
    if (output_file != NULL) {
        fprintf(output_file, "RLO_uniform_nopad_1000: %lf\n", rlo_value);
        fclose(output_file);
        printf("RLO_uniform_nopad value saved to RLO_uniform_nopad_1000.txt\n");
    }
    else {
