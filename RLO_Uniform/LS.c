...
double pr_rlen_s_given_rlen_x(int s, int x) {
    return 1.0 / s;  // s와 x에 관계없이 항상 1/s를 반환
}
...
void precompute_pr_rlen_prime_s(double* pr_rlen_prime_s, int* keyword_num_documents, int num_keywords, int s_min, int s_max) {
    double total_sum = 0.0;  // Pr(rlen'=s) 값들의 합을 계산하기 위한 변수

    // 각 s에 대해 서로 다른 두 개의 값의 합이 s가 되는 경우의 수를 계산
    for (int s = s_min; s <= s_max; s++) {
        int count = count_combinations(keyword_num_documents, num_keywords, s);  // 경우의 수 계산
        pr_rlen_prime_s[s - s_min] = count;  // Pr(rlen'=s) 값을 저장

        total_sum += count;  // 전체 합계에 추가
    }

    // Pr(rlen'=s) 값 정규화
    if (total_sum > 0) {
        for (int s = s_min; s <= s_max; s++) {
            pr_rlen_prime_s[s - s_min] /= total_sum;  // 각 값을 전체 합으로 나누어 정규화
        }
    }
}
